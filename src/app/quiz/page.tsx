...
import { Target, ClipboardCheck, Clock, AlertTriangle } from "lucide-react";
      ...
      <div style={{ borderBottom: `1px solid ${c.bd}`, background: c.gl, backdropFilter: "blur(16px)", position: "sticky", top: 0, zIndex: 50 }}>
        <Ctn style={{ padding: "12px 28px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}><Btn v="outline" sz="sm" onClick={() => { if (mode === "test" && !sub) sED(true); else if (mode === "practice") setExC(true); else router.push("/dashboard"); }}>Exit</Btn><span style={{ padding: "4px 11px", borderRadius: 8, background: c.acS, fontSize: 12.5, fontWeight: 600, border: `1px solid ${c.ac}12`, display: "inline-flex", alignItems: "center", gap: 5 }}>{mode === "practice" ? <><Target size={14} color={c.ac} /> Practice</> : <><ClipboardCheck size={14} color={c.ac} /> Mock Test</>}</span></div>
            <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
              {mode === "test" ? <Mono style={{ fontSize: 13.5, fontWeight: 700, color: tl < 300000 ? c.rd : c.fg, display: "inline-flex", alignItems: "center", gap: 4 }}><Clock size={14} color={tl < 300000 ? c.rd : c.fg} /> {`${Math.floor(tl / 60000)}:${String(Math.floor((tl % 60000) / 1000)).padStart(2, "0")}`}</Mono> : <span style={{ fontSize: 12.5, color: c.mt }}>{Math.floor((Date.now() - st) / 60000)} min</span>}
